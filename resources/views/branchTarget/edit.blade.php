...
@section('body')
    ...
    @endif
    <form method="post" action="{{route('branchTarget.update', $branchTarget->id)}}">
        @csrf
        @method('PUT')
        <br>
        ...
        <div class="form-row">
            <div class="col-md-6 mb-2">
                ...
                <select class="form-control select2bs4" id="branch_id" style="width: 100%;" name="branch_id" required>
                    ...
                    @foreach(\App\Models\Branch::all() as $branch)
                        <option value="{{$branch->id}}" @if($branchTarget->branch_id == $branch->id) selected @endif >{{$branch->code}} - {{$branch->region}} - {{$branch->zone}} - {{$branch->district}} - {{$branch->name}}</option>
                    @endforeach
            ...
            <div class="col-md-3 mb-2">
                ...
                <input type="number" step="0.01" value="{{$branchTarget->amount}}" id="amount" class="form-control" name="amount" required>
            </div>
            <div class="col-md-3 mb-3">
                <label for="year"><strong>Year</strong></label>
                <input type="date" class="form-control" value="{{$branchTarget->year}}" id="year" required name="year">
            </div>


        </div>

        <button class="btn btn-danger float-right " type="submit">Update Branch Target</button>
    </form>
@endsection
